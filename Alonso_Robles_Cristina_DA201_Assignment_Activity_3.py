...
# Searching for duplicates values in the ad DataFrame.
...
# Grouping the ad DataFrame by 'region_ons_code' and 'appointment_date'.
# And aggregating the 'count of appointments' to daily counts.

daily_appointments_count = pd.DataFrame(ad.groupby(['region_ons_code' ,'appointment_date']).count_of_appointments.sum().reset_index(name='total_daily_appointments'))
daily_appointments_count.sort_values(['total_daily_appointments'],ascending=False)
...
monthly_appointments_count = pd.DataFrame(nc.groupby(['icb_ons_code' ,'appointment_month']).count_of_appointments.sum().reset_index(name='total_monthly_appointments'))
monthly_appointments_count.sort_values(['total_monthly_appointments'],ascending=False)
